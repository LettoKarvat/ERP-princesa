        ...
        <Box sx={{ p: 2 }}>
            ...
            {CHECKLIST_ITEMS.map(item => {
                ...
                const bg = ans.answer === 'sim'
                    ? '#e0ffe0' : ans.answer === 'nao' ? '#ffe0e0' : 'inherit';
                ...
                return (
                    <Paper key={item.code} sx={{ p: 2, mb: 2, backgroundColor: bg }}>
                        ...
                        <FormControl>
                            ...
                            >
                                <FormControlLabel value="sim" label="Sim"
                                    control={<Radio sx={{ '&.Mui-checked': { color: 'green' } }} />} />
                                <FormControlLabel value="nao" label="Não"
                                    control={<Radio sx={{ '&.Mui-checked': { color: 'red' } }} />} />
                            </RadioGroup>
                        </FormControl>

                        <TextField
                            label="Observações" fullWidth multiline minRows={2} sx={{ mt: 2 }}
                            value={ans.obs}
            ...
            <Dialog
                open={openSignModal}
                onClose={(e, r) => { if (loading || r === 'backdropClick') return; setOpenSignModal(false); setSavedSignature(null); }}
                disableEscapeKeyDown={loading} maxWidth="sm" fullWidth
            >
                ...
                <DialogContent dividers>
                    ...
                    <Box sx={{ border: '1px solid #ccc', height: 200, mb: 2 }}>
                        <SignatureCanvas
                            ref={signatureRef} penColor="black"
                            canvasProps={{ width: 500, height: 200, style: { background: '#fff' } }}
                        />
                    </Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        <Button onClick={handleClearSignature} variant="outlined" disabled={loading}>Limpar</Button>
                        <Button onClick={handleSaveSignature} variant="outlined" disabled={loading}>Salvar</Button>
                    </Box>
                </DialogContent>
                <DialogActions>
                    <Button onClick={() => setOpenSignModal(false)} disabled={loading}>Cancelar</Button>
                    <Button variant="contained" onClick={handleConfirmSignature} disabled={loading}>Confirmar</Button>
                </DialogActions>
            </Dialog>
